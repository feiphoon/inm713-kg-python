...
class SPARQLEndpoint(object):
    ...
    def getQueryResultsArityOne(self, query, filter_uri=True):
        ...
        for result in results["results"]["bindings"]:
            ...
            if (
                not filter_uri
                or uri_value.startswith(URI_KG.dbpedia_uri)
                or uri_value.startswith(URI_KG.wikidata_uri)
                or uri_value.startswith(URI_KG.schema_uri)
                or uri_value.startswith(URI_KG.dbpedia_uri_resource)
                or uri_value.startswith(URI_KG.dbpedia_uri_property)
            ):
    ...
    def getQueryResultsArityTwo(self, query, filter_outA=True, filter_outB=True):
        ...
        for result in results["results"]["bindings"]:
            ...
            if (
                not filter_outA
                or outA_value.startswith(URI_KG.dbpedia_uri)
                or outA_value.startswith(URI_KG.wikidata_uri)
                or outA_value.startswith(URI_KG.schema_uri)
                or outA_value.startswith(URI_KG.dbpedia_uri_resource)
                or outA_value.startswith(URI_KG.dbpedia_uri_property)
            ):

                if (
                    not filter_outB
                    or outB_value.startswith(URI_KG.dbpedia_uri)
                    or outB_value.startswith(URI_KG.wikidata_uri)
                    or outB_value.startswith(URI_KG.schema_uri)
                    or outB_value.startswith(URI_KG.dbpedia_uri_resource)
                    or outB_value.startswith(URI_KG.dbpedia_uri_property)
                ):
    ...
    def createSPARQLQueryTriplesForObject(self, obj, limit=1000):
        ...
        return (
            "SELECT DISTINCT ?outA ?outB WHERE { ?outA ?outB <"
            + obj
            + "> . FILTER( ?outB NOT IN("
            + props_to_filter
            + ")) } limit "
    ...
    def createSPARQLQueryTriplesForSubject(self, subject, limit=1000):
        ...
        return (
            "SELECT DISTINCT ?outA ?outB WHERE { <"
            + subject
            + "> ?outA ?outB  FILTER( ?outA NOT IN("
            + props_to_filter
            + ")) } limit "
            + str(limit)
        )
    ...
    def createSPARQLQueryPredicatesForObject(self, obj, limit=1000):
        return (
            "SELECT DISTINCT ?uri WHERE { [] ?uri <" + obj + "> . } limit " + str(limit)
        )
    ...
    def createSPARQLQueryDomainTypesCountOfPredicatesForSubject(self, subject, limit=3):
        return (
            "SELECT DISTINCT ?uri WHERE { <"
            + subject
            + "> ?p [] . ?p rdfs:domain ?uri . } GROUP BY ?uri HAVING (COUNT(?uri) > 3) ORDER BY DESC(COUNT(?uri)) limit "
            + str(limit)
        )
    ...
    def createSPARQLQueryRangeTypesCountOfPredicatesForObject(self, obj, limit=3):
        return (
            "SELECT DISTINCT ?uri WHERE { [] ?p <"
            + obj
            + "> . ?p rdfs:range ?uri . } GROUP BY ?uri HAVING (COUNT(?uri) > 3) ORDER BY DESC(COUNT(?uri)) limit "
            + str(limit)
        )
    ...
    def createEnglishLabelsForURI(self, uri_subject):
        return (
            ...
            + uri_subject
            + "> rdfs:label ?literal . FILTER( langMatches(lang(?literal), 'en')) }"
        )
...
class DBpediaEndpoint(SPARQLEndpoint):
    ...
    def createSPARQLEntitiesForClass(self, class_uri, offset=0, limit=1000):
        ...
        return (
            "SELECT DISTINCT ?uri WHERE { ?uri <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <"
            + class_uri
            + "> . } ORDER BY RAND() OFFSET "
            + str(offset)
            + " limit "
            + str(limit)
        )
    ...
    def createSPARQLEntitiesLabelsForClass(self, class_uri, offset=0, limit=1000):
        ...
        return (
            "SELECT DISTINCT ?outA ?outB WHERE { ?outA <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <"
            + class_uri
            + "> . ?outA rdfs:label ?outB . FILTER( langMatches(lang(?outB), 'en')) } ORDER BY RAND() OFFSET "
            + str(offset)
            + " limit "
            + str(limit)
        )
    ...
    def createSPARQLQueryWikiPageRedirectsFrom(self, uri_object):
        ...
        return (
            "SELECT DISTINCT ?uri WHERE { ?uri <http://dbpedia.org/ontology/wikiPageRedirects> <"
            + uri_object
            + "> . }"
        )
    ...
    def createSPARQLQueryDistanceToAllSuperClassesForClass(self, uri_cls):
        return (
            "SELECT  ?outA (count(?mid) as ?outB) "
            + "WHERE {"
            + "<"
            + uri_cls
            + "> <http://www.w3.org/2000/01/rdf-schema#subClassOf>* ?mid . "
    ...
    def createSPARQLQueryDistanceToAllSubClassesForClass(self, uri_cls):
        ...
        return (
            "SELECT  ?outA (count(?mid) as ?outB) "
            + "WHERE {"
            + "?mid <http://www.w3.org/2000/01/rdf-schema#subClassOf>* <"
            + uri_cls
            + "> . "
...
class WikidataEndpoint(SPARQLEndpoint):
    ...
    def createSPARQLEntitiesForClass(self, class_uri, offset=0, limit=1000):
        ...
        return (
            "SELECT DISTINCT ?uri WHERE { ?uri <http://www.wikidata.org/prop/direct/P31> <"
            + class_uri
            + "> . } ORDER BY RAND() OFFSET "
            + str(offset)
            + " limit "
    ...
    def createSPARQLQueryAllTypesForSubject(self, uri_subject):
        ...
        return (
            ...
            + "{<"
            + uri_subject
            + "> <http://www.wikidata.org/prop/direct/P31> ?dt . "
    ...
    def createSPARQLQueryEquivalentClasses(self, uri_class):
        ...
        return (
            ...
            + "{<"
            + uri_class
            + "> <http://www.wikidata.org/prop/direct/P1709> ?uri ."
            + "} "
            + "UNION {"
            ...
            + "} "
            + "UNION {"
            + "?uri <http://www.wikidata.org/prop/direct/P1709> <"
            + uri_class
            + "> ."
            + "} "
            + "UNION {"
            + "<"
            + uri_class
            + "> <http://www.wikidata.org/prop/direct/P2888> ?uri . }"
            + "}"
        )
    ...
    def createSPARQLQuerySameAsEntities(self, uri_entity):
        return (
            ...
            + "{<"
            + uri_entity
            + "> <http://www.wikidata.org/prop/direct/P460> ?uri ."
            + "} "
            + "UNION {"
            + "?uri <http://www.wikidata.org/prop/direct/P460> <"
            + uri_entity
            + "> ."
            + "} "
            + "}"
    ...
    def createSPARQLQueryAllSuperClassesForClass(self, uri_cls):
        ...
        return (
            ...
            + "<"
            + uri_cls
            + "> <http://www.wikidata.org/prop/direct/P279>+ ?uri "
            + "}"
        )
    ...
    def createSPARQLQueryAllSubClassesForClass(self, uri_cls):
        ...
        return (
            "SELECT DISTINCT ?uri "
            + "WHERE {"
            + "?uri <http://www.wikidata.org/prop/direct/P279>+ <"
            + uri_cls
            + ">"
            + "}"
        )
...
if __name__ == "__main__":
    ...
    ent = "http://dbpedia.org/resource/Democratic_Republic_of_the_Congo"
    ep = DBpediaEndpoint()
    ...
    ent = "http://www.wikidata.org/entity/Q974"  # Democratic Republic of the Congo
    ep = WikidataEndpoint()
    types = ep.getTypesForEntity(ent)
    print(len(types), types)
